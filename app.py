...
# --- 1. 기본 설정 및 한글 폰트 ---
st.set_page_config(page_title="공공데이터 대시보드", layout="wide")

# 한글 깨짐 방지 (환경에 따라 'Malgun Gothic' 또는 'AppleGothic' 사용)
...
if page == "축제 현황":
    ...
    # [데이터 준비] 지표값이 세로로 쌓인 데이터를 가로로 펼침(Pivot)
    query1 = """
    SELECT "개최년도", "축제명",
    ...
    df_2024['type'] = df_2024.apply(get_type, axis=1)

    # 시각화 1: 요약 카드 + 산점도 (통합 Figure)
    ...
    ax_card.axis('off')
    ax_card.set_title("2024년 축제기간 기준 · 점 크기 = 축제지 집중률", loc='left', pad=20, fontsize=10, color='gray')
    
    years = [2022, 2023, 2024]
    for i, year in enumerate(years):
        # 해당 연도 당일치기형 개수 (2024 중앙값 기준)
        y_df = df_fest[df_fest['개최년도'] == year]
        count = len(y_df[(y_df['inflow_rate'] >= mx) & (y_df['consumption_idx'] < my)])
    ...
    for t, color in colors.items():
        sub = df_2024[df_2024['type'] == t]
        ax_scatter.scatter(sub['inflow_rate'], sub['consumption_idx'], s=sub['concentration']*20, c=color, label=t, alpha=0.7)
    
    ax_scatter.axvline(mx, color='gray', ls='--', lw=1)
    ax_scatter.axhline(my, color='gray', ls='--', lw=1)
    ...
    st.pyplot(fig1)
    with st.expander("사용 SQL 및 인사이트"):
        st.code(query1)
        st.write("① 인사이트: ")
...
# ==========================================
    ...
    # 데이터 로드 (공실률, 임대료, 매핑, 축제지표 병합)
    q_v = 'SELECT "상권명", ("2024_Q2" - "2022_1Q") as v_diff FROM "임대동향 지역별 공실률 소규모 상가"'
    q_r = 'SELECT "상권명", ("2024_Q2" - "2022_1Q") as r_diff FROM "임대동향 지역별 임대료 소규모 상가"'
    q_m = 'SELECT "상권명", "연동자치단체명" FROM "매핑테이블"'
    q_i = 'SELECT "축제명", AVG("지표값") as inflow FROM "문화관광축제주요지표" WHERE "구분명"=\'외부방문자 유입\' GROUP BY "축제명"'
    
    df_gent = run_query(q_m).merge(run_query(q_v), on='상권명').merge(run_query(q_r), on='상권명')
    ...
    ax_card.axis('off')
    exp_df = df_gent[df_gent['type'] == '축제 상권']
    avg_v, avg_r = exp_df['v_diff'].mean(), exp_df['r_diff'].mean()
    ...
    sns.scatterplot(data=df_gent, x='v_diff', y='r_diff', hue='type', palette={'축제 상권': '#D85A30', '일반 상권': '#1D9E75'}, s=100, ax=ax_scatter)
    ax_scatter.axvline(df_gent['v_diff'].median(), ls='--', color='gray')
    ax_scatter.axhline(df_gent['r_diff'].median(), ls='--', color='gray')
    ax_scatter.set_title("2024년 축제기간 기준 상권 변화", loc='left', fontsize=10)
    ...
    st.subheader("차트 2. 예산 통제 시 상권 변화 검증")
    st.pyplot(fig4)
...
# ==========================================
    ...
    ax5.barh(df_tax['행사·축제명'], df_tax['recovery_rate'], color=[tax_color(x) for x in df_tax['recovery_rate']])
    ax5.set_title("축제별 세금 회수율 (2024, %)", loc='left')
    
    st.subheader("차트 1. 수평 막대 차트")
    st.pyplot(fig5)
    with st.expander("사용 SQL 및 인사이트"):
        st.code(query5)
        st.write("인사이트: ")

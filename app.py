...
def render_page2():
    ...
    # 차트 2 버블 크기 계산: (지자체 예산규모(백만원)) * 100 공식 적용 (안정적 가시성 확보 목적 하한값 10 세팅) [1]
    df_relation["점크기_예산"] = df_relation["예산(백만원)"] * 100
    df_relation.loc[df_relation["점크기_예산"] < 10, "점크기_예산"] = 10
    ...
    fig2 = px.scatter_3d(
        ...
        y="공실률변화량",
        z="예산(백만원)",
        size="점크기_예산",  # 보정된 버블 크기 적용
        color="상권구분",
        # 차트 2에서 상권명 가려짐 및 난잡함을 줄이기 위해 라벨 지움 조치
        color_discrete_map={
            "축제 상권 (실험군)": "#FF4B4B",
            "일반 상권 (대조군)": "#1F77B4"
        },

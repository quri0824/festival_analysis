...
# 헬퍼 함수: 표준 2글자 광역 지역명 통일 매핑
def get_clean_2char_region(text):
    ...
    mapping = {
        ...
        "충남": ["천안", "공주", "보령", "아산", "서산", "논산", "계룡", "당진", "금산", "부여", "서천", "청양", "홍성", "예산", "태안", "강경", "머드"],
        "전북": ["전주", "군산", "익산", "정읍", "남원", "김제", "완주", "진안", "무주", "장수", "임실", "순창", "고창", "부안", "지평선"],
    ...
    for reg, cities in mapping.items():
...
# 헬퍼 함수: 축제 지표 데이터 가로(Wide) 구조 표준화
def standardize_festival_df(df):
    col_gubun = find_col(df.columns, ["구분명", "구분", "지표명"])
    col_val = find_col(df.columns, ["지표값", "값", "실적"])
    col_name = find_col(df.columns, ["축제명", "행사명", "축제"]) or df.columns[0]
    
    # 1) 세로형(Long) 구조인 경우 가로형(Wide) 피벗 수행
    if col_gubun and col_val:
        df_clean = df.copy()
        df_clean[col_val] = pd.to_numeric(df_clean[col_val], errors='coerce').fillna(0)
        
        index_cols = [c for c in df.columns if c not in [col_gubun, col_val]]
        
        # 중복 방지를 위한 그룹화 후 피벗
        df_grouped = df_clean.groupby(index_cols + [col_gubun])[col_val].mean().reset_index()
        df_pivoted = df_grouped.pivot(index=index_cols, columns=col_gubun, values=col_val).reset_index()
        
        if "지자체" not in df_pivoted.columns:
            df_pivoted["지자체"] = df_pivoted[col_name].apply(get_clean_2char_region)
            
        return df_pivoted
        
    # 2) 이미 가로형(Wide)인 경우 지자체 컬럼만 추가/표준화
    df_res = df.copy()
    if "지자체" not in df_res.columns:
        reg_col = detect_region_col(df_res)
        df_res["지자체"] = df_res[reg_col or col_name].apply(get_clean_2char_region)
    else:
        df_res["지자체"] = df_res["지자체"].apply(get_clean_2char_region)
...
def render_page1():
    ...
    df_fest = standardize_festival_df(df_fest)
...
def render_page2():
    ...
    df_prop["매칭키"] = df_prop[reg_col_vac].apply(get_clean_2char_region)
    
    # 3) 축제 규모 연동 및 고유 키(매칭키) 기준으로 평균화 그룹화 (중복 방지)
    ...
    df_fest_clean["매칭키"] = df_fest_clean[fest_reg].apply(get_clean_2char_region)
    
    # 축제 데이터가 지역별 1:N 매핑되어 산점도가 중복 생성되는 문제를 막기 위해 groupby 처리
    df_fest_group = df_fest_clean.groupby("매칭키")[foreign_col].mean().reset_index()
    df_fest_group.columns = ["매칭키", "외부방문자유입"]
    
    # 4) 지자체 총 예산액 연동 및 고유 키(매칭키) 기준으로 합계 그룹화 (중복 방지)
    ...
    df_cost_clean["매칭키"] = df_cost_clean[cost_org].apply(get_clean_2char_region)
    
    # 예산 데이터가 지역별 1:N 매핑되는 문제를 막기 위해 groupby 처리
    df_cost_group = df_cost_clean.groupby("매칭키")[cost_val].sum().reset_index()
    df_cost_group.columns = ["매칭키", "예산총액(원)"]
    
    # 5) 종합 조인 (1:1 병합으로 중복 데이터 생성 차단)
    ...
    # 임대료 변화율 및 공실률 변화량 소수점 셋째자리 버림 처리 함수 적용
    def truncate_to_2_dec(val):
        if pd.isna(val):
            return 0.0
        return math.trunc(val * 100) / 100

    df_relation["임대료변화율"] = df_relation["임대료변화율"].apply(truncate_to_2_dec)
    df_relation["공실률변화량"] = df_relation["공실률변화량"].apply(truncate_to_2_dec)
    
    df_relation["상권구분"] = df_relation["외부방문자유입"].apply(
        lambda x: "축제 상권 (실험군)" if x > 0 else "일반 상권 (대조군)"
    )
    
    # 외부방문자 유입 * 100을 버블 크기로 정의
    df_relation["점크기_방문자"] = df_relation["외부방문자유입"] * 100
    df_relation.loc[df_relation["점크기_방문자"] < 5, "점크기_방문자"] = 8  # 최소 가시 버블 크기 보존
    ...
    # ------------------------------------------
    # 차트 1번: 임대료 변화율 x 공실률 변화 산점도 (가독성 개편 버전)
    # ------------------------------------------
    st.subheader("📊 차트 1: 임대료 변화율 × 공실률 변화 사분면 매트릭스")
    st.write("각 영역별 사분면 명칭 가이드를 참조하여 지역별 상권 변화 양상을 비교할 수 있습니다.")
    ...
    fig1 = px.scatter(
        ...
        labels={
            ...
            "점크기_방문자": "외부방문자 유입 가중치"
        },
        template="plotly_white"
    )
    
    # 4사분면 해석을 돕기 위한 텍스트 레이블 및 가이드 박스 추가 (가독성 향상)
    annotations = [
        dict(x=0.85, y=0.85, xref="paper", yref="paper", text="⚠️ 젠트리피케이션 위험<br>(임대료↑ 공실률↑)", showarrow=False, font=dict(color="red", size=11), bgcolor="rgba(255,230,230,0.6)"),
        dict(x=0.15, y=0.85, xref="paper", yref="paper", text="📉 상권 쇠퇴 구역<br>(임대료↓ 공실률↑)", showarrow=False, font=dict(color="orange", size=11), bgcolor="rgba(255,245,230,0.6)"),
        dict(x=0.15, y=0.15, xref="paper", yref="paper", text="✅ 상권 안정 영역<br>(임대료↓ 공실률↓)", showarrow=False, font=dict(color="green", size=11), bgcolor="rgba(230,250,230,0.6)"),
        dict(x=0.85, y=0.15, xref="paper", yref="paper", text="📈 상권 활성 영역<br>(임대료↑ 공실률↓)", showarrow=False, font=dict(color="blue", size=11), bgcolor="rgba(230,240,255,0.6)")
    ]
    for ann in annotations:
        fig1.add_annotation(ann)
    ...
    fig1.update_traces(
        ...
        marker=dict(line=dict(width=1.5, color='DarkSlateGrey'), opacity=0.85)
    )
    ...
    m_vac_full["매칭키"] = m_vac_full[r_v_col].apply(get_clean_2char_region)
    m_rent_full["매칭키"] = m_rent_full[r_r_col].apply(get_clean_2char_region)
    
    m_vac_full = pd.merge(m_vac_full, df_fest_group[["매칭키", "외부방문자유입"]], on="매칭키", how="left")
    m_vac_full["상권구분"] = m_vac_full["외부방문자유입"].apply(lambda x: "축제 상권 (실험군)" if x > 0 else "일반 상권 (대조군)")
    
    m_rent_full = pd.merge(m_rent_full, df_fest_group[["매칭키", "외부방문자유입"]], on="매칭키", how="left")
    m_rent_full["상권구분"] = m_rent_full["외부방문자유입"].apply(lambda x: "축제 상권 (실험군)" if x > 0 else "일반 상권 (대조군)")
...
def render_page3():
    ...
    df_sub["매칭키"] = df_sub[org_col].apply(get_clean_2char_region)
    
    df_f_map = df_fest_clean[[fest_reg, foreign_col]].copy()
    df_f_map.columns = ["지자체명", "외부방문자"]
    df_f_map["매칭키"] = df_f_map["지자체명"].apply(get_clean_2char_region)
    
    # 세로형 피벗화 이후 고유 키별 외부방문자 수 취합 ( groupby )
    df_f_map_group = df_f_map.groupby("매칭키")["외부방문자"].mean().reset_index()
    
    df_roi = pd.merge(df_sub, df_f_map_group, on="매칭키", how="left")

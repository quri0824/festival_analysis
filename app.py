...
# 동적 엔진 2: 상권명 세부 컬럼 검출
def detect_detailed_district_col(df):
    name_match = find_col(df.columns, ["상권명", "상권", "소분류", "지점"])
    if name_match:
        return name_match
    return None


# 동적 엔진 3: 년도/ID를 제외한 첫 번째 유효한 수치형 컬럼 검출
...
# 소수점 특정 자리수 정밀 버림 가공 함수
def truncate_float(val, decimals=2):
    if pd.isna(val) or val is None:
        return 0.0
    try:
        val = float(val)
        # 이진 부동소수점 오차를 방지하기 위해 넉넉한 정밀도의 스트링으로 변환 후 끊기
        s = f"{val:.6f}"
        before_dec, after_dec = s.split('.')
        truncated_after = after_dec[:decimals]
        return float(f"{before_dec}.{truncated_after}")
    except Exception:
        return val


# 가로 형태 데이터를 세로 형태로 변환 (다중 인덱스 보강 지원)
...
def get_fallback_property_vacancy():
    return pd.DataFrame({
        ...
        "상권명": ["춘천역", "논산 화지시장", "김제 상권", "명동", "수원역", "부평", "해운대", "동성로"],
...
def render_page1():
    ...
    df_bsi = pd.DataFrame({
        ...
        "체감 BSI": [
            44.31, 37.39, 55.07, 75.59, 74.62, 65.50,
            53.41, 58.57, 71.83, 62.81, 57.79, 58.61,
            48.30, 47.96, 61.86, 63.20, 63.83, 62.41,
            59.08, 63.39, 70.76, 69.71, 63.88, 60.04,
            48.15, 51.45, 64.95, 65.45, 63.79, 58.10,
            53.61, 54.81, 64.59, 69.34, 63.65, 53.81
        ],
        "전망 BSI": [
            66.41, 69.59, 83.20, 90.81, 101.13, 86.83,
            75.18, 71.45, 91.61, 90.78, 79.81, 83.18,
            67.27, 66.30, 81.41, 81.21, 84.46, 81.69,
            76.31, 84.49, 97.65, 95.06, 89.61, 85.93,
            78.81, 65.40, 75.13, 78.61, 74.39, 67.04,
            57.45, 56.36, 81.49, 83.51, 80.43, 79.59
        ]
    })
...
def render_page2():
    ...
    # 광역 지자체 컬럼 및 세부 상권명 컬럼 추출
    reg_col_vac = detect_region_col(df_vac)
    district_col_vac = detect_detailed_district_col(df_vac) or reg_col_vac
    
    reg_col_rent = detect_region_col(df_rent)
    district_col_rent = detect_detailed_district_col(df_rent) or reg_col_rent
    
    # 1) 공실률 변화량 연산 및 버림 가공
    df_vac_calc = df_vac[[reg_col_vac, district_col_vac, first_q, last_q]].copy()
    ...
    df_vac_calc["공실률변화량"] = df_vac_calc["공실률변화량"].apply(lambda x: truncate_float(x, 2))
    
    # 2) 임대료 변화율 연산 및 버림 가공
    df_rent_calc = df_rent[[reg_col_rent, district_col_rent, first_q, last_q]].copy()
    ...
    # 임대동향 테이블 간 텍스트 불일치 예방 목적 안전 폴백 매커니즘 (세부 상권 결합 실패 시 시도 매칭)
    if df_prop.empty:
        df_prop = pd.merge(
            df_vac_calc[[reg_col_vac, district_col_vac, "공실률변화량"]], 
    ...
    # [정교화된 시군구 단위 매칭 시스템] 축제가 직접 개최되는 대상 도시명을 정확히 추출하여 필터링
    ...
    # 시군구 단위를 통과한 진정한 실험군 상권과, 지차체 내 축제 영향권이 아닌 대조군 상권을 명확히 재분류
    ...
    # X축 표출 범위가 [-4, 4]이므로 배경 음영 블록 역시 시작과 끝점을 맞추어 정류
    fig1.add_vrect(
        x0=0, x1=4,
    ...
    # 세부 상권 고유 코드를 축약하지 않고 다중 인덱스로 멜팅 처리하여 정보 탈락 방지
